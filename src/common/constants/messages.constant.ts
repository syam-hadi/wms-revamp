export const Messages = {
  COMMON: {
    SUCCESS: 'Success.',

    CREATED: 'Data created successfully.',
    UPDATED: 'Data updated successfully.',
    DELETED: 'Data deleted successfully.',

    NOT_FOUND: 'Data not found.',
    DUPLICATE: 'Data already exists.',

    INVALID_REQUEST: 'Invalid request.',
    UNAUTHORIZED: 'Unauthorized.',
    FORBIDDEN: 'Forbidden.',
  },

  CONFIG: {
    CREATED: 'Configuration created successfully.',
    UPDATED: 'Configuration updated successfully.',
    DELETED: 'Configuration deleted successfully.',

    NOT_FOUND: 'Configuration not found.',
    DUPLICATE_CODE: 'Configuration code already exists.',
  },

  COMPANY: {
    CREATED: 'Company created successfully.',
    UPDATED: 'Company updated successfully.',
    DELETED: 'Company deleted successfully.',

    NOT_FOUND: 'Company not found.',
    DUPLICATE_CODE: 'Company code already exists.',
  },

  BRANCH: {
    CREATED: 'Branch created successfully.',
    UPDATED: 'Branch updated successfully.',
    DELETED: 'Branch deleted successfully.',

    NOT_FOUND: 'Branch not found.',
    DUPLICATE_CODE: 'Branch code already exists.',
  },

  DEPARTMENT: {
    CREATED: 'Department created successfully.',
    UPDATED: 'Department updated successfully.',
    DELETED: 'Department deleted successfully.',

    NOT_FOUND: 'Department not found.',
    DUPLICATE_CODE: 'Department code already exists.',
  },

  CUSTOMER: {
    CREATED: 'Customer created successfully.',
    UPDATED: 'Customer updated successfully.',
    DELETED: 'Customer deleted successfully.',

    NOT_FOUND: 'Customer not found.',
    DUPLICATE_CODE: 'Customer code already exists.',
  },

  VENDOR: {
    CREATED: 'Vendor created successfully.',
    UPDATED: 'Vendor updated successfully.',
    DELETED: 'Vendor deleted successfully.',

    NOT_FOUND: 'Vendor not found.',
    DUPLICATE_CODE: 'Vendor code already exists.',
  },

  EMPLOYEE: {
    CREATED: 'Employee created successfully.',
    UPDATED: 'Employee updated successfully.',
    DELETED: 'Employee deleted successfully.',

    NOT_FOUND: 'Employee not found.',
    DUPLICATE_NIK: 'Employee NIK already exists.',
  },

  COUNTRY: {
    CREATED: 'Country created successfully.',
    UPDATED: 'Country updated successfully.',
    DELETED: 'Country deleted successfully.',

    NOT_FOUND: 'Country not found.',
    DUPLICATE_CODE: 'Country code already exists.',
  },

  PROVINCE: {
    CREATED: 'Province created successfully.',
    UPDATED: 'Province updated successfully.',
    DELETED: 'Province deleted successfully.',

    NOT_FOUND: 'Province not found.',
    DUPLICATE_CODE: 'Province code already exists.',
  },

  CITY: {
    CREATED: 'City created successfully.',
    UPDATED: 'City updated successfully.',
    DELETED: 'City deleted successfully.',

    NOT_FOUND: 'City not found.',
    DUPLICATE_CODE: 'City code already exists.',
  },

  BANK: {
    CREATED: 'Bank created successfully.',
    UPDATED: 'Bank updated successfully.',
    DELETED: 'Bank deleted successfully.',

    NOT_FOUND: 'Bank not found.',
    DUPLICATE_SHORT_NAME: 'Bank short name already exists.',
    DUPLICATE_NAME: 'Bank name already exists.',
  },

  TAX: {
    CREATED: 'Tax created successfully.',
    UPDATED: 'Tax updated successfully.',
    DELETED: 'Tax deleted successfully.',

    NOT_FOUND: 'Tax not found.',
    DUPLICATE_NAME: 'Tax name already exists.',
  },
} as const;
